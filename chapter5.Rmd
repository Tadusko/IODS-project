...
factor_variable_names <- names(Filter(is.factor, tea_time))
#num_plots <- length(factor_variable_names)
#par(mfrow = c(ceiling(sqrt(num_plots)), ceiling(sqrt(num_plots))))

for (variable in factor_variable_names) {
  barplot(table(tea_time[[variable]]), main = variable, col = "skyblue", cex.main = 0.8)
}
